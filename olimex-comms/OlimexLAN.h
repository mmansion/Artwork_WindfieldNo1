...
//TODO: integrate
  boolean bits[16]; //https://forum.arduino.cc/t/array-of-bits-in-c/119183/2
...
void OlimexLAN::checkUDP() {  
  ...
  if(packetSize) {
    ...
    for (uint8_t packet_n = 0; packet_n < packetSize; packet_n++) {
      byte in_byte = int(this->packetBuffer[packet_n]);
      for(uint8_t bit_n = 0; bit_n < 8; bit_n++) {
        if(debug_udp) Serial.print(bitRead(in_byte, bit_n));
        bits[bit_n] = bitRead(in_byte, bit_n);
      }
    }

//    this->onMessageReceived(*bits);
